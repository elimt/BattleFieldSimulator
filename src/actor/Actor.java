 ...
 */
public abstract class Actor implements Serializable { // Could also be written as " public class Actor extends Object { " but no one ever does . . . why waste time typing something that isn't
																											// required.
	...
	public void startMotion(boolean engageInCombat) {
		...
		if (opponent != null) {
			...
			// opponent.getAvatar().getTranslateX());
			// *************************** Start Your work for findNewLocation() *******************************
			// You must implement the findNewLocation() methods for each subclass
			// The newly created Point2D object will contain the newly calculated X/Y values
